...
| Uninstall the hook while the filter is disabled | The only gain is that observation stops, and nothing observed leaves the process or is recorded without a measurement session. The cost is real: reinstalling on Enable is a failing operation that would need its own tray and dialog error path, hook-health would need a disabled special case, and keeping Raw Input up to preserve measurement halves the gain anyway. The bypass is one branch returning `PassThrough` |
