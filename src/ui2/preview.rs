...
//! transport controls. The Iced sources are `src/ui/app.rs::timing_preview`
//! and the `preview::clock` widget in `src/ui/preview.rs`; the transport
//! glyphs are procedural paths ported from `src/ui/icons.rs`, so the card
...
/// The Iced `ArrowLeft`/`ArrowRight` glyph (`src/ui/icons.rs:407-423`), used
...
mod tests {
    ...
    // The data-model tests ported from `src/ui/preview.rs`.
